...
TRAIN_START = "20180101"   # 训练集窗口结束日开始日期（含）
...
TRAIN_SHARD_IDS = [2,3,4]     # 训练分片 ID 列表（空=前 TRAIN_SHARDS 片）
...
FINE_TUNE_SHARD_WEIGHTS = [2.5, 3.0] # 分片级时间权重（从旧到新，与 LAST_SHARDS 一一对应；最后一片权重最高=最偏近期）
FINE_TUNE_EPOCHS = 5                 # 微调 epoch（小，防遗忘）
FINE_TUNE_LR = 1e-5                  # 微调学习率（主训练 9e-5 的约 1/10）
